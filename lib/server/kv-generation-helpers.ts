/**
 * 主视觉生成共享服务端工具：
 *  - `buildKvPrompt`：GPT 拼接玩法 PE + 参考图 hint，吐出 nanobanana 可直接消化的中文 prompt。
 *  - `buildKvReferenceImages` / `appendCampaignNanoRefTail`：根据玩法决定 nanobanana 参考图顺序与尾注。
 *  - Spec schema：与 `app/api/generate-kv/route.ts` 共用，保证 Agent 与 Custom 两套链路同源。
 *
 * 该文件被 `app/api/generate-kv` 与新的 `generate-kv-pe / generate-kv-from-prompt` 共用，
 * 修改时请同时跑这三条链路的 build 与端到端冒烟。
 */
import { z } from "zod";
...
export const ChongbangSpecSchema = z
  .object({
...
export const StarCollectSpecSchema = z
  .object({
    targetLanguage: z.string().optional(),
...
export const WheelSpecSchema = z
  .object({
    targetLanguage: z.string().optional(),
    scene: z.string().optional(),
    prizeElements: z.string().optional(),
    projectileBrief: z.string().optional(),
    gridThemeBrief: z.string().optional(),
    decorativeElements: z.string().optional(),
    primaryColor: z.string().optional(),
    ipBrief: z.string().optional(),
    coinVariation: z.string().optional(),
...
export const TuijinbiSpecSchema = WheelSpecSchema;

export type BuildKvPromptParams = {
  theme: string;
...
export async function buildKvPrompt(params: BuildKvPromptParams): Promise<string> {
  ...
  if (params.campaignType === "chongbang") {
    ...
    systemAndTaskBlock =
      `你是广告视觉 Prompt Engineer。我会提供主题、参考图与**推金币(踢球进网格)**专用 PE。你要输出**一段**可直接给 nanobanana 执行的中文生图 prompt。\n\n` +
      `【推金币 · 附件顺序务必对齐】本轮生图库的参考图附件顺序为 **版式 → 画风(图2) → IP(图3) → 金币/代币**。下文「图2/图3」标签与附件顺序一致；**不要**套用「画风在 IP 之后」的旧惯性。\n\n` +
      `【推金币玩法】下文专用 PE 为「推金币/踢球进网格」换皮书；关于**版图结构锁定、格子数量与外框拓扑、发射区与主副按钮占位、画风与叙事**等与通用条款并行时以前文专用 PE + 本条为准。**图 1 无二维码区时不要虚构可扫满幅码**。**禁止**把标尺、画布测距写入成片可读区。\n\n` +
      ...
      `${KV_PROMPT_SYSTEM}\n\n---\n\n${pe}\n\n---\n\n` +
      `须同时体现："踢球进网格"同一套玩法页骨架 + 点击下方发射命中上方目标区并入池得金币的阅读感；全页基调统一。\n\n` +
  ...
  if (params.images.layoutBase64) {
    ...
    if (params.campaignType === "chongbang") {
      ...
      layoutHint =
        "图1：「推金币/踢球进网格」版式母版(`public/` 下 **kv-tuijinbi-1.png～kv-tuijinbi-4.png** 四张为已修正的最终参考)——**纵向分区与按钮布局锁定，仅换肤**。无上传时程序随机附其中一张；有上传则以**仅此一张上传图**为栅格。**顶栏、中部奖格板、台前池与 Hero、底栏主钮+左右圆钮**的相对位置与占画比例须与附图一致；**奖格 M×N 仅以本张为准**(内置常见：-1 为 3×4=12；-2/-3/-4 多为 4×4=16，仍须向图 1 数清)，不得套错格数、不得混用其它内置图的阵列行列。首句写明版图锁定。";
    } else if (params.campaignType === "baiyuan") {
      layoutHint =
    ...
    const styleHint =
      params.campaignType === "wheel"
        ...
        : params.campaignType === "tuijinbi"
          ? "图2：画风与环境母题参考(**推金币强约束**)。目标气质常为**亮色、可爱 Q 版三维**。从图中读出**渲染维度、光照与调色盘、材质颗粒与铬边习惯**，写进最终 prompt；成片**阵列外框 / 滑道与台面 / 球形或币形奖励 / 底控制区按键**的总体材质与色相须与该张**同一套三维管线**，**版图分区仍 100% 跟图 1**。须锁定：主辅色、冷暖光、高光形状、边缘虚实；**禁止**在参考明显为某一文化场景时强行换成另一整套地域陈设模板；不要照搬图2 的具体阵列行列或物体摆放。你输出的生图 prompt 正文须写出**至少 5 条**对照图2 的可核验风格锚点。"
  ...
  if (params.images.ipBase64) {
    ...
    if (params.campaignType === "chongbang") {
      ...
      ipHint =
        "图3：IP 角色参考(**用户已上传则必达**)。须与图为**同一角色**而非「同色动物」：逐条复现**五官走向(眉/眼/鼻/嘴)、头身比与耳位、毛色分块与服饰结构、标志性小道具**；**必须**在最终 prompt 中写明图3 的**阴影/材质画法**(如粗线勾边、高光对比、半色调网点、平涂腮红等)，**禁止**改成另一套 Q 版比例。推金币玩法中 IP **仅配角装饰**，**不得遮挡**中部 **图1 阵列各格可读图标区**，不得挡大号前景 Hero；可出现于顶栏、角标或小装饰。正文须列**至少 5 条**可对照图3 的具体外观锚点，不得仅写「参考图3」或泛称。";
    } else if (params.campaignType === "baiyuan") {
      ipHint =
...
export function buildWheelNanoPromptTail(
...
export function buildTuijinbiNanoPromptTail(
  refs: ReadonlyArray<{ name: string }>
): string {
  if (refs.length <= 1) return "";
  const label: Record<string, string> = {
    layout:
      "版式母版(推金币：`public/` 下 kv-tuijinbi-1～4 四选一或用户上传——纵向分区与底栏三钮锁定；奖格 M×N 以本张为准，常见 -1 为 12 格、另三张多为 16 格；仅换材质/插画/配色/文案)",
    style:
      "画风全局参考——成片光照、主辅色、边缘光与材质颗粒须与该张一致；阵列外框/台面/发射物与按钮饰面须服从，禁止换掉版图骨架或挪动按钮三相关系",
    ip: "IP 角色参考——须为同一吉祥物/IP(禁止换物种、换脸或另一套配色分区)；仅配角位，严禁遮挡图1阵列各格可读区与 Hero",
    coin: "金币或代币质感与镶边参考",
  };
...
export function buildWheelLocaleBlob(
  theme: string,
  wheelSpec: WheelKvSpec | null | undefined
): string {
  const parts = [theme];
  if (wheelSpec) {
...
export type KvReferenceImage = { name: string; base64: string; mimeType: string };

/**
 * 按玩法决定参考图顺序：转盘/推金币为 版式→画风→IP→金币；其它玩法 版式→IP→画风→金币。
 * 入参中的 base64 可带 dataURL 前缀（内部统一裁剪）。
 */
export function buildKvReferenceImages(
  campaignType: KvCampaignType,
  imagesWithLayout: KvImages
): KvReferenceImage[] {
  if (!imagesWithLayout.layoutBase64) {
    throw new Error("buildKvReferenceImages: layoutBase64 是必传项");
  }
  const layoutPure = stripDataUrlPrefix(imagesWithLayout.layoutBase64);
  const layoutMime = getImageMimeFromBase64(imagesWithLayout.layoutBase64);
  const layoutEntry: KvReferenceImage = {
    name: "layout",
    base64: layoutPure,
    mimeType: layoutMime,
  };
  const styleEntry: KvReferenceImage | null = imagesWithLayout.styleBase64
    ? {
        name: "style",
        base64: stripDataUrlPrefix(imagesWithLayout.styleBase64),
        mimeType: getImageMimeFromBase64(imagesWithLayout.styleBase64),
      }
    : null;
  const ipEntry: KvReferenceImage | null = imagesWithLayout.ipBase64
    ? {
        name: "ip",
        base64: stripDataUrlPrefix(imagesWithLayout.ipBase64),
        mimeType: getImageMimeFromBase64(imagesWithLayout.ipBase64),
      }
    : null;
  const coinEntry: KvReferenceImage | null = imagesWithLayout.coinBase64
    ? {
        name: "coin",
        base64: stripDataUrlPrefix(imagesWithLayout.coinBase64),
        mimeType: getImageMimeFromBase64(imagesWithLayout.coinBase64),
      }
    : null;

  const styleBeforeIp = campaignType === "wheel" || campaignType === "tuijinbi";
  const ordered = styleBeforeIp
    ? [layoutEntry, styleEntry, ipEntry, coinEntry]
    : [layoutEntry, ipEntry, styleEntry, coinEntry];
  return ordered.filter((x): x is KvReferenceImage => x !== null);
}

/**
 * 根据玩法返回 nanobanana prompt 尾注；对非 wheel/tuijinbi 玩法仅在用户上传了 IP 参考时拼接。
 */
export function buildCampaignNanoRefTail(
  campaignType: KvCampaignType,
  refs: ReadonlyArray<{ name: string }>,
  hasIpUpload: boolean
): string {
  if (campaignType === "wheel") return buildWheelNanoPromptTail(refs);
  if (campaignType === "tuijinbi") return buildTuijinbiNanoPromptTail(refs);
  return hasIpUpload ? ipPromptTailByCampaign[campaignType] : "";
}
